...
const navModel = {
  ...
  // 移动端导航项（可选：可以和 mainItems 相同，也可以自定义）
  mobileItems: [
    {
      name: 'About',
      path: '/about',
  ...
  // 扩展方法：动态添加导航项
  addItem(name, path, icon, label) {
    const iconSvg = this.getIconSvg(icon);
    this.mainItems.push({ name, path, icon, iconSvg, label });
    this.mobileItems.push({ name, path, icon, label });
  },
  ...
  getIconSvg(iconName) {
    const icons = {
      ...
      folder: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>',
      ...
      shopping: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>',
